...
class TargetsTimeout(RuntimeError): ...
...
class TargetsService:
    ...
    def targets(self, region: str) -> list[Species]:
        ...
        self.client.beta.sessions.events.send(
            ...
            events=[
                {
                    "type": "user.message",
                    "content": [{"type": "text", "text": f"targets:{region}"}],
                }
            ],
            betas=_BETA,
        )
            ...
            for event in events.data:
                if event.type == "agent.message":
                    ...
                    if isinstance(payload, dict) and "error" in payload:
                        raise TargetsError(payload["error"])
                    return payload
            self.sleeper(self.poll_interval_s)

        raise TargetsTimeout(f"no agent.message after {self.max_polls} polls")
