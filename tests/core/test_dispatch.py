...
def test_register_command_attaches_on_message_once():
    ...
    register_command(bot, "!rares", handler)  # type: ignore[arg-type]
...
def test_register_command_two_prefixes_share_single_on_message():
    ...
    register_command(bot, "!rares", search_handler)  # type: ignore[arg-type]
    first_on_message = bot.registered["on_message"]
...
async def test_dispatcher_routes_prefixes_to_their_handlers():
    ...
    search_msg = FakeMessage("!rares US-CA")
...
async def test_two_separate_bots_have_independent_registries():
    ...
    register_command(bot_a, "!rares", handler_a)  # type: ignore[arg-type]
    ...
    assert "!rares" in bot_a._command_registry  # type: ignore[attr-defined]
    ...
    assert "!rares" not in bot_b._command_registry  # type: ignore[attr-defined]
